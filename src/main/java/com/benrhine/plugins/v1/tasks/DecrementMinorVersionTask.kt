...
/** --------------------------------------------------------------------------------------------------------------------
 * DecrementMinorVersionTask: Decrement the minor version.
 * ------------------------------------------------------------------------------------------------------------------  */
open class DecrementMinorVersionTask : DefaultTask() {
    /**
     * decrementMinorVersion: Task that executes incrementing the minor version.
     *
     * @throws Exception (optionally)
     */
    @TaskAction
    @Throws(Exception::class)
    fun decrementMinorVersion() {
        try {
            val prop = getLocalProperties(project)
            // Increment the minor version and store it into the local properties object
            val minor = prop.getProperty("minor").toInt()
            if (minor > 0) {
                prop.setProperty("minor", (minor - 1).toString())
            } else {
